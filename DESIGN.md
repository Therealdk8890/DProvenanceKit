...
- **One silent loss remains: a failed batch insert.** Congestion shedding and encode failures are now counted by tier (`TraceDropStats` / `store.dropStats.preservedIntegrity`, §5), so a consumer can tell a genuine absence from a dropped event. The gap still open is in the background writer: if a batch `INSERT` throws, those rows have already been drained from the buffer, so they are logged but not counted — a write-time failure can still thin a run without showing up in `dropStats`. Closing it means accounting for drain-after-dequeue loss in the writer, and is the next correctness investment.
